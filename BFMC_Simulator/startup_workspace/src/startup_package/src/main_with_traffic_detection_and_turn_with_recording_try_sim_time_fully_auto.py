...
def traffic_light_reaction(car_postion):
	...
	if closest_traffic_sign == "TL0":
		if traffic_status == 2:
			...
			print(traffic_id, "status: open ")
		else:
			# stop the car
			car.drive(0, get_current_steer())
			print(traffic_id, "status: not open yet ")
	elif  traffic_status == 0:
		# move the car
		car.drive( 0.4 , get_current_steer() )
		print(traffic_id, "status: open ")
	else :
...
def images_thread():
	...
	model_turn_go_ahead = load_model('model_round_go_ahead_150_v2.h5')
	...
	model_round_recover = load_model('model_round_road_recover_150_v4.h5')
	...
	while True:
		...
		if driving_mode == 1:
			...
			if abs( current_yaw - last_yaw) > 1.3:
		...
		# means a round mode
			...
			# when the counter reaches 150 the car will finish the round mode
			if turn_counter < 20:
				pred_steering = float(model.predict(image))
				print("first part if round mode is active")
			elif turn_counter < 180:
				# predict the steering angles
				pred_steering = float(model_round_model.predict(image))
				print("second part of round mode is active")
			elif turn_counter < 240:
				# predict the steering angles
					...
					if confidence > 0.8:
						...
						# if the object close and on the center
							...
							# 3) if its a car
								...
								elif ((center_y + h/2) > (2 * height/3) and (allow_car_passing == False)) and (driving_mode != 7):
							...
							# # 5) if it a end parking line
							# elif object_detected =='end_parking_line':
							# 	# when it is the end of parking spot
							# 	 if driving_mode == 7:
							# 		# send commands to the car and send the same steering but
							# 		print("End parking mode, car stopped")
							# 		car.drive(0.0, get_current_steer())
