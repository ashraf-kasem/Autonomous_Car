...
def img_preprocess(img, driving_mode):
	# lane keeping, making a turn mode and car over taking
	if driving_mode == 3 :
		img = img[200:,:,:]
	# if the mode is turning left and right
	elif driving_mode in [1,2]:
		img = img[120:, :, :]
	# if go a head
	else :
		img = img[150:,:,:]
...
def images_thread():
	...
	model_turn_right = load_model('model_turn_right.h5')
	# load the  turn left model
	model_turn_left = load_model('model_turn_left.h5')
	# load the  turn go ahead model
	model_turn_go_ahead = load_model('model_go_ahead.h5')
	# load the change lane left
	model_change_lane_left = load_model('model_change_lane_left.h5')
	# load the take over
	model_change_lane_right = load_model('model_switch_lan_right.h5')
